...
namespace P_detect_events {
    QVector<double> align_events(const QVector<double>& X, const QVector<double>& ptimes, int sign, double detect_interval)
    {
        int winaround = 10; // samples before/after rms peak to look for filtered peak (hard-coded for now)
        int winlen = winaround * 2;
        bigint N = ptimes.size();
        QVector<double> times;
        double maxv = 0; // peak amplitude
        double maxind = 0; // peak position
        double maxindprev = 0; // previous peak position
        double val = 0; // current value
    ...
    QVector<double> detect_events(const QVector<double>& X, double detect_threshold, double detect_interval, int sign)
    {
        ...
        //double stdev2 = MLCompute::median(X)/0.6745; Ref: Quiroga et al. 2004
        //double threshold2 = detect_threshold * stdev2; 
